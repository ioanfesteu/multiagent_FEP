...
These three parameters, located in `agents.py` under **Psycho-behavioral Parameters**, control the physics of the body, the dynamics of emotion, and the impact of mood on behavior.

Here is the breakdown of what each parameter does in the context of the Solms-Friston Active Inference model:

### 1. *ETA = 0.1* (Thermal Coupling / Permeability)
...
In Code (agents.py):
```python
# The agent's body temp moves 10% of the way towards the environment temp per step
...
### 2. *MU_AFFECT = 0.1* (Emotional Inertia)
...
### 3. *SIGMA = 1.0* (Precision Sensitivity)
